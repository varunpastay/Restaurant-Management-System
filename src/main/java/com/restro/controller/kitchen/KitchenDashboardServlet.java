...
/** Renders the kitchen dashboard shell; the live order queue itself is fetched and refreshed via AJAX (KitchenOrdersApiServlet). */
@WebServlet(name = "KitchenDashboardServlet", urlPatterns = {"/kitchen/dashboard"})
public class KitchenDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.getRequestDispatcher("/WEB-INF/jsp/kitchen/dashboard.jsp").forward(request, response);
        } catch (SQLException e) {
            throw new ServletException("Failed to load kitchen dashboard", e);
        }
    }
}
